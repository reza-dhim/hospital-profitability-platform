...
Cost, Revenue, Driver, Asset, Employee, Medical Activity, BMHP, Tariff — one Excel template per type (per `PRD.md`), **all implemented** (`apps/api/src/upload/upload.constants.ts`'s `SUPPORTED_UPLOAD_TYPES`). Two distinct promotion shapes: Cost/Revenue/Driver/Medical Activity are period-scoped, append-only case-level or period-aggregate data — hard-delete rollback, scoped by `source_file_id`. Asset/Employee/BMHP/Tariff bulk-*create* the master-data entities of the same name — insert-only (a duplicate `code` is a validation error, not an upsert; corrections go through Master Data CRUD, not a re-upload), soft-delete rollback, except Tariff which supersedes on create (same as `TariffService.create()`, restored on rollback). Medical Activity (Sprint 8) is the source of `service_direct_cost` (`docs/10_UNIT_COST_ENGINE.md` §2) and `doctor_profitability_results` (`docs/11_DOCTOR_ANALYTICS.md` §2). Each implemented template is versioned (`template_version` embedded as a hidden header row) so the engine can detect and reject uploads against a stale template.
