...
import type { DoctorProfitabilityEngineService } from "../doctor-analytics/doctor-profitability-engine.service";
...
function makeDeps() {
  const tx = {
    ...
    doctorProfitabilityResult: { createMany: jest.fn().mockResolvedValue({ count: 0 }) },
  };
  ...
  const prisma = {
    ...
    medicalActivity: { groupBy: jest.fn().mockResolvedValue([]) },
  ...
  const doctorProfitabilityEngineService = {
    computeRows: jest.fn().mockResolvedValue([]),
  } as unknown as DoctorProfitabilityEngineService;

  return { prisma, tx, tenantContextService: new TenantContextService(), targetMarginService, doctorProfitabilityEngineService };
}

describe("ProfitabilityEngineService.processRun", () => {
  it("is a no-op when the run is not in 'completed' status", async () => {
    const { prisma, tx, tenantContextService, targetMarginService, doctorProfitabilityEngineService } = makeDeps();
    (prisma.allocationRun.findFirst as jest.Mock).mockResolvedValue({ ...completedRun, status: "running" });
    const service = new ProfitabilityEngineService(prisma, tenantContextService, targetMarginService, doctorProfitabilityEngineService);
  ...
  it("is a no-op when the run doesn't exist for this hospital", async () => {
    const { prisma, tenantContextService, targetMarginService, doctorProfitabilityEngineService } = makeDeps();
  ...
  describe("service_unit_costs (docs/10_UNIT_COST_ENGINE.md)", () => {
     ...
     * service_direct_cost = 0 for both (no medical_activities rows in this test).
    ...
    /**
     * MANUAL CALCULATION (Sprint 8): PC-RJ has two services, only one
     * (SVC-A) has medical_activities data this period. SVC-A: allocated
     * cost 4,000,000 (equal split, both revenue-less), direct cost =
     * 100,000+200,000+50,000 = 350,000, medical_activities volume = 8 (NOT
     * the revenue_entries volume, which is absent here entirely). SVC-B has
     * no medical_activities rows at all -> direct cost stays 0, volume
     * falls back to revenue_entries (also absent here -> 0).
     *   SVC-A total_cost = 4,000,000 + 350,000 = 4,350,000; unit_cost = 4,350,000/8 = 543,750
     *   SVC-B total_cost = 4,000,000 + 0 = 4,000,000; volume = 0 -> unit_cost = null
     */
    it("wires service_direct_cost from medical_activities and prefers its own volume, per service — a service with no medical_activities rows still falls back to revenue_entries (0 here)", async () => {
      const { prisma, tx, tenantContextService, targetMarginService, doctorProfitabilityEngineService } = makeDeps();
      (prisma.profitCenter.findMany as jest.Mock).mockResolvedValue([{ id: "PC-RJ" }]);
      (prisma.allocatedCost.groupBy as jest.Mock).mockResolvedValue([{ targetProfitCenterId: "PC-RJ", _sum: { amount: 8_000_000 } }]);
      (prisma.service.findMany as jest.Mock).mockResolvedValue([
        { id: "SVC-A", profitCenterId: "PC-RJ", currentTariff: null },
        { id: "SVC-B", profitCenterId: "PC-RJ", currentTariff: null },
      ]);
      (prisma.medicalActivity.groupBy as jest.Mock).mockResolvedValue([
        { serviceId: "SVC-A", _sum: { bmhpCost: 100_000, roomCost: 200_000, staffCost: 50_000, volume: 8 } },
      ]);
      const service = new ProfitabilityEngineService(prisma, tenantContextService, targetMarginService, doctorProfitabilityEngineService);
      ...
      expect(byServiceId["SVC-A"]).toMatchObject({
        serviceAllocatedCost: "4000000.00",
        serviceDirectCost: "350000.00",
        serviceVolume: "8.00",
        unitCost: "543750.0000",
      });
      expect(byServiceId["SVC-B"]).toMatchObject({
        serviceAllocatedCost: "4000000.00",
        serviceDirectCost: "0.00",
        serviceVolume: "0.00",
        unitCost: null,
  ...
  describe("doctor_profitability_results (docs/11_DOCTOR_ANALYTICS.md, Sprint 8)", () => {
    it("passes serviceAllocatedCostByServiceId from the unit-cost stage into DoctorProfitabilityEngineService, and writes its rows in the same transaction", async () => {
      const { prisma, tx, tenantContextService, targetMarginService, doctorProfitabilityEngineService } = makeDeps();
      (prisma.profitCenter.findMany as jest.Mock).mockResolvedValue([{ id: "PC-RJ" }]);
      (prisma.allocatedCost.groupBy as jest.Mock).mockResolvedValue([{ targetProfitCenterId: "PC-RJ", _sum: { amount: 8_000_000 } }]);
      (prisma.service.findMany as jest.Mock).mockResolvedValue([{ id: "SVC-A", profitCenterId: "PC-RJ", currentTariff: null }]);
      const doctorRows = [
        {
          allocationRunId: "run-1",
          doctorId: "doc-1",
      ...
      (doctorProfitabilityEngineService.computeRows as jest.Mock).mockResolvedValue(doctorRows);
      const service = new ProfitabilityEngineService(prisma, tenantContextService, targetMarginService, doctorProfitabilityEngineService);

      await service.processRun(payload);

      // Single service in PC-RJ -> 100% apportionment -> serviceAllocatedCost = 8,000,000.
      const [hospitalIdArg, allocationRunIdArg, periodIdArg, serviceAllocatedCostByServiceId] = (
        doctorProfitabilityEngineService.computeRows as jest.Mock
      ).mock.calls[0] as [string, string, string, Map<string, { toFixed: (n: number) => string }>];
      expect(hospitalIdArg).toBe("hospital-1");
      expect(allocationRunIdArg).toBe("run-1");
      expect(periodIdArg).toBe("period-1");
      expect(serviceAllocatedCostByServiceId.get("SVC-A")!.toFixed(2)).toBe("8000000.00");

      expect(tx.doctorProfitabilityResult.createMany).toHaveBeenCalledWith({ data: doctorRows });
    });

    it("does not call createMany for doctor_profitability_results when there are no rows", async () => {
      const { tx, prisma, tenantContextService, targetMarginService, doctorProfitabilityEngineService } = makeDeps();
      const service = new ProfitabilityEngineService(prisma, tenantContextService, targetMarginService, doctorProfitabilityEngineService);

      await service.processRun(payload);

      expect(tx.doctorProfitabilityResult.createMany).not.toHaveBeenCalled();
    });
  });
});
