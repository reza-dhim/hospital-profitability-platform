...
function makeDeps() {
  ...
  return { prisma, passwordService, tokenService, permissionsService };
}
...
describe("AuthService.login", () => {
  it("issues tokens and persists a refresh token for valid credentials", async () => {
    const { prisma, passwordService, tokenService, permissionsService } = makeDeps();
    (prisma.user.findUnique as jest.Mock).mockResolvedValue(baseUser);
    (passwordService.verify as jest.Mock).mockResolvedValue(true);

    const service = new AuthService(prisma, passwordService, tokenService, permissionsService);
